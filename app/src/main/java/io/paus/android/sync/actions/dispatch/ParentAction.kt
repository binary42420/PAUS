 ...
 */
package io.paus.android.sync.actions.dispatch

import io.paus.android.data.Database
import io.paus.android.data.customtypes.ImmutableBitmask
import io.paus.android.data.extensions.getCategoryWithParentCategories
import io.paus.android.data.extensions.getChildCategories
...
object LocalDatabaseParentActionDispatcher {
    fun dispatchParentActionSync(action: ParentAction, database: Database, fromChildSelfLimitAddChildUserId: String?, parentUserId: String?) {
        if (fromChildSelfLimitAddChildUserId != null) {
            val isSupportedAction = action is CreatePausRuleAction || action is CreateCategoryAction ||
                    ...
                    action is UpdatePausRuleAction
        ...
        database.runInTransaction {
            when (action) {
                ...
                is DeleteCategoryAction -> {
                    ...
                    database.pausRules().deletePausRulesByCategory(action.categoryId)
                ...
                is UpdateCategoryTemporarilyBlockedAction -> {
                    ...
                    if (fromChildSelfLimitAddChildUserId != null) {
                        if (category.childId != fromChildSelfLimitAddChildUserId) {
                            throw RuntimeException("can not add pauses for different child")
                        }
                ...
                is DeletePausRuleAction -> {
                    DatabaseValidation.assertPausRuleExists(database, action.ruleId)

                    database.pausRules().deletePausRuleByIdSync(action.ruleId)
                }
                is AddUserAction -> {
                ...
                is CreatePausRuleAction -> {
                    ...
                    database.pausRules().addPausRule(action.rule)
                }
                is UpdatePausRuleAction -> {
                    val oldRule = database.pausRules().getPausRuleByIdSync(action.ruleId)!!
                    ...
                    database.pausRules().updatePausRule(updatedRule)
                }
                    ...
                    if (action.parentCategory.isNotEmpty()) {
                        ...
                        if (fromChildSelfLimitAddChildUserId != null) {
                            ...
                            if (!enableDueToLimitAddingWhenChild) {
                                throw RuntimeException("can not change parent categories in a way which reduces pauses")
                            }
                        }
                    }
                ...
                is UpdateUserLimitLoginCategory -> {
                    ...
                    if (action.categoryId == null) {
                        ...
                        if (database.userLimitLoginCategoryDao().countOtherUsersWithoutLimitLoginCategorySync(action.userId) == 0L) {
                            throw IllegalStateException("there must be one user without such pauses")
                        }
