...
import io.paus.android.sync.validation.ListValidation
...
data class CreatePausRuleAction(val rule: PausRule): ParentAction()

data class UpdatePausRuleAction(
...
data class DeletePausRuleAction(val ruleId: String): ParentAction() {
    init {
        IdGenerator.assertIdValid(ruleId)
    }
}
