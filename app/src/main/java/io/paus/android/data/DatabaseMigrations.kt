...
object DatabaseMigrations {
    ...
    val MIGRATE_TO_V14 = object: Migration(13, 14) {
        override fun migrate(database: SupportSQLiteDatabase) {
            database.execSQL("ALTER TABLE `paus_time_rule` ADD COLUMN `start_minute_of_day` INTEGER NOT NULL DEFAULT ${PausRule.MIN_START_MINUTE}")
            database.execSQL("ALTER TABLE `paus_time_rule` ADD COLUMN `end_minute_of_day` INTEGER NOT NULL DEFAULT ${PausRule.MAX_END_MINUTE}")
            database.execSQL("ALTER TABLE `paus_time_rule` ADD COLUMN `session_duration_milliseconds` INTEGER NOT NULL DEFAULT 0")
            database.execSQL("ALTER TABLE `paus_time_rule` ADD COLUMN `session_pause_milliseconds` INTEGER NOT NULL DEFAULT 0")
    ...
    val MIGRATE_TO_V18 = object: Migration(17, 18) {
        override fun migrate(database: SupportSQLiteDatabase) {
            database.execSQL("ALTER TABLE `category` ADD COLUMN `disable_pauses_until` INTEGER NOT NULL DEFAULT 0")
        }
    }
    ...
    val MIGRATE_TO_V20 = object: Migration(19, 20) {
        override fun migrate(database: SupportSQLiteDatabase) {
            database.execSQL("ALTER TABLE `paus_time_rule` ADD COLUMN `per_day` INTEGER NOT NULL DEFAULT 0")
        }
    }
