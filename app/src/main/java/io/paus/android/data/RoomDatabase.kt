...
import io.paus.android.data.dao.DerivedDataDao
import io.paus.android.data.invalidation.Observer
import io.paus.android.data.invalidation.Table
import io.paus.android.data.invalidation.TableUtil
...
abstract class RoomDatabase: RoomDatabase(), io.paus.android.data.Database {
    companion object {
        private val lock = Object()
        private var instance: io.paus.android.data.Database? = null
        const val DEFAULT_DB_NAME = "db"
        const val BACKUP_DB_NAME = "db2"

        fun with(context: Context): io.paus.android.data.Database {
            if (instance == null) {
                synchronized(lock) {
                    if (instance == null) {
        ...
        fun createInMemoryInstance(context: Context): io.paus.android.data.Database {
            return Room.inMemoryDatabaseBuilder(
                    context,
                    io.paus.android.data.RoomDatabase::class.java
            ).build()
        }

        fun createOrOpenLocalStorageInstance(context: Context, name: String): io.paus.android.data.Database {
