 ...
 */
package io.paus.android.ui.manage.category.paus_rules.edit
...
import io.paus.android.databinding.FragmentEditPausRuleDialogBinding
import io.paus.android.extensions.MinuteOfDay
import io.paus.android.extensions.getParcelableCompat
...
import io.paus.android.sync.actions.DeletePausRuleAction
...
import io.paus.android.ui.view.SelectDayViewHandlers
...
class EditPausRuleDialogFragment : BottomSheetDialogFragment() {
    companion object {
        ...
        fun newInstance(existingRule: PausRule, selfLimitMode: Boolean, listener: Fragment) = EditPausRuleDialogFragment()
            .apply {
        ...
        fun newInstance(categoryId: String, selfLimitMode: Boolean, listener: Fragment) = EditPausRuleDialogFragment()
            .apply {
                arguments = Bundle().apply {
    ...
    private var existingRule: PausRule? = null
    private var savedNewRule: PausRule? = null
    private var selfLimitMode: Boolean = false
    private lateinit var newRule: PausRule
    private lateinit var view: FragmentEditPausRuleDialogBinding
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val listener = targetFragment as EditPausRuleDialogFragmentListener
        val database = DefaultAppLogic.with(requireContext()).database

        view = FragmentEditPausRuleDialogBinding.inflate(layoutInflater, container, false)
        ...
        run {
            val restoredRule = savedInstanceState?.getParcelable<PausRule>(STATE_RULE)
        ...
        view.handlers = object: Handlers {
            ...
            override fun updateSessionLength() {
                DurationPickerDialogFragment.newInstance(
                        titleRes = R.string.category_paus_time_rules_session_limit_duration,
            ...
            override fun updateSessionBreak() {
                DurationPickerDialogFragment.newInstance(
                        titleRes = R.string.category_paus_time_rules_session_limit_pause,
            ...
            override fun onSaveRule() {
                ...
                if (existingRule != null) {
                    if (existingRule != newRule) {
                        val updateAction = UpdatePausRuleAction(
                            ruleId = newRule.id,
        ...
        if (existingRule != null) {
            database.pausRules()
                    .getPausRuleByIdLive(existingRule!!.id).observe(viewLifecycleOwner, Observer {
                        if (it == null) {
                            // rule was deleted
                            dismissAllowingStateLoss()
                        } else {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        childFragmentManager.setFragmentResultListener(REQUEST_START_TIME_OF_DAY, viewLifecycleOwner) { _, bundle ->
            ...
            if (!MinuteOfDay.isValid(minuteOfDay)) {
                ...
                Toast.makeText(context, R.string.category_paus_time_rules_invalid_range, Toast.LENGTH_SHORT).show()
            } else {
...
interface EditPausRuleDialogFragmentListener {
    fun notifyRuleCreated()
    fun notifyRuleDeleted(oldRule: PausRule)
    fun notifyRuleUpdated(oldRule: PausRule, newRule: PausRule)
}